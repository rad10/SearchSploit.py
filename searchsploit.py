...
def highlightTerm(line: str, term) -> str:
    ...
    if type(term) is tuple:
        # TODO: Make way to highlight line if fits version parameters
        term = term[1]
...
def getVersion(term: str):
    # returns the first found object, taking tuple out of list
    return baseVersion.findall(term)
...
def cmpVersion(leftVersion: str, rightVersion: str) -> int:
    ...
    # if they are equal up their shortest version, then the latest is the longest version
    return len(rightComponent) - len(leftComponent)
                    ...
                    if type(term) is str:  # separate versions from search terms
                        ...
                        if (term[0] != "") and "<" not in term[0]:  # version has r or s in front
                            for v in versions:
                                ...
                                versionCompatible = versionCompatible or (
                                    cmpVersion(term[1], v[1]) >= 0)
